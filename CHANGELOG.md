...
- 📈 **多周期趋势补全** — 市场数据新增 `日线 / 周线 / 月线` 三周期摘要，输出最新收盘、单周期涨跌、区间涨跌、5 周期均线和趋势判断，减少仅凭日线判断中期趋势的误差
- 💰 **A 股估值历史分位** — 基本面数据新增 `市盈率(TTM)`、`市净率` 的历史分位表，补充当前值、历史中位数、历史极值和样本数，便于判断估值是否已经反映业绩变化

### 改进

- 🇭🇰 **港股估值容灾增强** — 当 `yfinance` 在中国大陆环境下被限流时，港股仍会尝试从亿牛网补充 PE/PB 历史序列，尽量避免再次出现“估值数据缺失”
- 🧭 **数据输出更适合 LLM 判断** — 行情与基本面文本结构补充了可直接推理的中周期趋势和估值定位，降低报告里“无法确认是否便宜 / 周线月线无数据”的情况
