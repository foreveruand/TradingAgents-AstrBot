...
- 📐 **估值历史分位** — A 股默认补充 PE / PB 历史分位，辅助判断当前价格是否已反映业绩变化
- 🗓️ **多周期趋势补全** — 输出日线、周线、月线三个周期的趋势摘要，减少“只看日线”的判断偏差
- 🧭 **证据驱动报告** — 提示词要求给出结论、证据、触发条件、失效条件、适用周期、仓位和止损/减仓参考，减少空泛结论
...
- 从 `1.2.4` 起，运行时提示词也会附加报告质量约束，即使已有旧人格未被覆盖，报告仍会要求引用数据、区分周期、给出确认/失效条件和风险控制动作。
...
- A 股估值历史分位当前优先使用 `stock_zh_valuation_baidu`；港股估值历史在 `yfinance` 不可用时，会额外尝试 `stock_hk_indicator_eniu` 作为补充。
